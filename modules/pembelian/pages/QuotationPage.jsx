...
function QuotationPage({ rows, setRows }) {
  const columns = [
    ...
    { key:'Status', label:'Status', render:(v,r) => quotationStatusPill(r) },
  ];
  ...
  const save = (form, mode) => {
    ...
    window.__erpToast && window.__erpToast('Quotation berhasil disimpan.');
  ...
  const cancelDoc = (form) => {
    ...
    setModal(updated);
    window.__erpToast && window.__erpToast('Quotation dibatalkan.');
  };
  ...
  const completeDoc = (form) => {
    ...
    setRows(prev => prev.map(r => r.No_Bukti===updated.No_Bukti ? updated : r));
    setModal(updated);
    window.__erpToast && window.__erpToast('Quotation diselesaikan manual.');
  };
  ...
  return (
    <>
      <PbDocList
        ...
        addLabel="Quotation Baru"
        statusFilter={statusFilter}
      />
      {showCetak && (
        <PbCetakModal docLabel="Quotation" rows={rows} statusFilter={statusFilter}
      ...
      {confirmCancel && (
        <ConfirmationModal
          ...
          message={`Quotation "${confirmCancel.No_Bukti}" akan dibatalkan. Transaksi yang sudah dibatalkan tidak bisa diaktifkan kembali.`}
