// Pembelian — halaman Request for Quotation (RFQ)
...
function RfqPage({ rows, setRows }) {
  const columns = [
    ...
    { key:'Status', label:'Status', render:(v,r) => rfqStatusPill(r) },
  ];
  ...
  const save = (form, mode) => {
    ...
    window.__erpToast && window.__erpToast('RFQ berhasil disimpan.');
  ...
  const cancelDoc = (form) => {
    ...
    setModal(updated);
    window.__erpToast && window.__erpToast('RFQ dibatalkan.');
  };
  ...
  const completeDoc = (form) => {
    ...
    setRows(prev => prev.map(r => r.No_Bukti===updated.No_Bukti ? updated : r));
    setModal(updated);
    window.__erpToast && window.__erpToast('RFQ diselesaikan manual.');
  };
    ...
    <>
      ...
      {confirmCancel && (
        <ConfirmationModal
          ...
          message={`RFQ "${confirmCancel.No_Bukti}" akan dibatalkan. Transaksi yang sudah dibatalkan tidak bisa diaktifkan kembali.`}
